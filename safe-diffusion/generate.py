...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument('--pipeline_type', type=str, default="sd", choices=["sd", "sld", "sega", "sdd"])
    parser.add_argument('--pipeline_config', type=str, default=None)
    parser.add_argument('--safety_concept', type=str, default=None)
    
    parser.add_argument('--image_dir', type=str, default='outputs/')
    ...
    parser.add_argument('--save_path', default=None, help='dummy')
    parser.add_argument('--model_name', default=None, help='dummy')
...
def validate(
    ...
    if seeds is not None:
        # assert args.num_samples == 1, "Cannot use seeds with num_samples > 1. Please duplicate the prompt with different seeds."
    ...
    # Duplicate prompts and seeds if num_samples > 1
    _prompts = [(i, p) for i, p in enumerate(prompts) for _ in range(args.num_samples)]
    prompts = [p[1] for p in _prompts]
    indices = [p[0] for p in _prompts]

    if args.image_dir is not None:
        if args.coco:
            args.image_dir = args.image_dir + '_coco'
        if args.experiment is not None and args.experiment.split('_')[-1].strip() == 'i2p':
            args.image_dir = args.image_dir + '_i2p'
        os.makedirs(os.path.join(args.image_dir, args.experiment, str(args.seed)), exist_ok=True)
    ...
    for i in range(num_total_iters):
        ...
        if args.save_path is not None:
            for image in images:
                image_file_name = os.path.join(os.path.join(args.save_path, args.experiment, str(args.seed)), f"{index:06d}.png")
                image.save(image_file_name)
                print(f'Saved at {image_file_name}')
                all_image_files.append(image_file_name)
                index += 1
        elif args.image_dir is not None:
            for image in images:
                image_file_name = os.path.join(os.path.join(args.image_dir, args.experiment, str(args.seed)), f"{index:06d}.png")
                image.save(image_file_name)
                print(f'Saved at {image_file_name}')
                all_image_files.append(image_file_name)
                index += 1
...
def main():
    ...
    # This will allow the saved sdd models to be loaded
    if args.pipeline_type == 'sdd':
        if args.experiment is not None:
            experiment_split = args.experiment.split('_')
            if experiment_split[-1] == 'i2p':
                args.experiment = experiment_split[0] 
    
    if args.save_path is not None and args.experiment is None:
        os.makedirs(args.save_path, exist_ok=True)
        print('HERE')
        args.experiment = args.save_path.split('_')[-1]
        if args.experiment == 'cat':
            args.experiment = 'grumpy'
        os.makedirs(os.path.join(args.save_path, args.experiment, str(args.seed)), exist_ok=True)
    args.pretrained_model_name_or_path = os.path.join('saved', args.experiment)
