...
def preprocess(opt, prompts, outdir, concept_type, ranks):
    ...
    if concept_type == 'memorization':
        ...
        # generate new images
        print(opt.train_size)
        print(len(anchor_prompts))
        ...
        with open(opt.prompts, "r") as f:
            data = f.read().splitlines()
            # Make it so we have modulus for opt.n_samples
            print(f'previous data length: {len(data)}')
            num_to_remove = len(data) % opt.n_samples
            if num_to_remove > 0:
                num_to_remove = opt.train_size % len(data)
            if num_to_remove > 0:
                data = data[:-num_to_remove]
            opt.train_size = len(data)
            print(f'opt.train_size: {opt.train_size}')
