...
def parse_arguments():
    ...
    parser.add_argument('--safety', type=str, default='ti_sd',
                        choices=['ti_sd', 'i_clip', 't_text', "t_match", "i_image", "i_dogcat", "custom_ti", "ud"], help='the safety filter applied to the system.')
    ...
    parser.add_argument('--seed',  type=int, default=None,
                        help='Fix the noise seed in Diffusion, default is None')
    ...
    parser.add_argument('--prompts_path',type=str, default = 'data/nsfw_list.txt') # data/nsfw_list_sample.txt
...
def main():
    ...
    saved_figure_path = f'figure/{todaystr}-{args.method}-{args.target}-{args.reward_mode}-{args.safety}-{args.len_subword}-{args.threshold}-{args.prompts_with_nud}-{args.prompts_with_clean}-{args.seed}-{args.ourclip_ckpt is not None}'
    os.makedirs(time.strftime(saved_figure_path), exist_ok=True)
    os.makedirs(time.strftime(f'{saved_figure_path}'), exist_ok=True)

    if args.target == 'sd':
        pipe = SDPipeline(torch_device, args.safety, fix_seed=args.seed, prompts_with_nud=args.prompts_with_nud, prompts_with_clean=args.prompts_with_clean, ourclip_ckpt=args.ourclip_ckpt)
    else:
        pipe = DL2Pipeline(torch_device)

    target_prompt_list = load_data(args.prompts_path)
    ...
    for i in target_prompt_list:
        ...
        if target_nsfw == False:
            ...
            pilimage[0].save(f"{saved_figure_path}/{args.target_prompt[0][0:20]}.png")
            print(f'Saved here: {f"{saved_figure_path}/{args.target_prompt[0][0:20]}.png"}')
    ...
    os.makedirs("results_adapt", exist_ok = True)
    results_df.to_csv(f'results_adapt/{todaystr}_{args.method}_{args.target}_{args.reward_mode}_{args.safety}_{args.len_subword}_{args.threshold}.csv',index=False)
    print(f'==> Statistic results saved under "results_adapt/"')
